...
void* KNN(void *args1) {
    thread_arguments *args = (thread_arguments*) args1;
...
int main(int argc, char *argv[]){
    ...
    int data_per_thread = test->num_instances() / THREAD_COUNT;
    int leftover_data = test->num_instances() % THREAD_COUNT;
    ...
    for(int i = 0; i < THREAD_COUNT; i++) {
        args = (thread_arguments*) malloc(sizeof(thread_arguments));

        if(leftover_data > 0) {
            data_end = data_start + data_per_thread + 1;
            leftover_data = leftover_data - 1;
        } else {
            data_end = data_start + data_per_thread;
        }
        ...
        pthread_create(&threads[i], NULL, KNN, (void*) args);
